...
PROVIDED_FC_OPTIONS;PROVIDED_FC_SUBMODULES;\
...
function(deps_make_resolver MODE)
    ...
    elseif(MODE STREQUAL "conan")
        set(_chain "conan;apt;provided")
    elseif(MODE STREQUAL "vcpkg")
        set(_chain "vcpkg;apt;provided")
...
function(find_dependency_unified NAME)
    ...
    if(_handler)
        deps_lock_record(${NAME} ${_handler})
    endif()
    if(NOT _handler)
        if(ARG_REQUIRED)
            get_property(_chain GLOBAL PROPERTY deps.chain)
...
# --- Lockfile (records resolved handler + version per dep for reproducibility) ---

function(deps_lock_record NAME HANDLER)
    get_property(_ver GLOBAL PROPERTY "deps.rec.${NAME}.VERSION")
    set_property(GLOBAL APPEND PROPERTY deps.lock.names "${NAME}")
    set_property(GLOBAL PROPERTY "deps.lock.${NAME}.handler" "${HANDLER}")
    set_property(GLOBAL PROPERTY "deps.lock.${NAME}.version" "${_ver}")
endfunction()

function(deps_lock_write)
    set(_file "${CMAKE_BINARY_DIR}/deps.lock.json")
    get_property(_names GLOBAL PROPERTY deps.lock.names)
    file(WRITE "${_file}" "{\n  \"dependencies\": {\n")
    set(_first TRUE)
    foreach(_n ${_names})
        get_property(_h GLOBAL PROPERTY "deps.lock.${_n}.handler")
        get_property(_v GLOBAL PROPERTY "deps.lock.${_n}.version")
        if(_first)
            set(_first FALSE)
        else()
            file(APPEND "${_file}" ",\n")
        endif()
        file(APPEND "${_file}" "    \"${_n}\": { \"handler\": \"${_h}\", \"version\": \"${_v}\" }")
    endforeach()
    file(APPEND "${_file}" "\n  }\n}\n")
    if(DEPS_DEBUG)
        message(STATUS "[deps] lockfile written to ${_file}")
    endif()
endfunction()
